...
class PCGA:
    ...
    def __init__(self, forward_model, s_init, pts, params, s_true = None, obs = None, obs_true = None, X = None):
        ...
        #Continue?
        self.Continue= params['Continue']  #kxy added
...
#print(Psi_sigma)
    ...
    def GaussNewton(self):
        ...
        # simul_obs = np.copy(simul_obs_init) # marked by kxy

        if self.Continue: #added by kxy
            s_past=np.loadtxt('ContinueK.txt')    #######need change here! hard code
            s_past = s_past.reshape(-1, 1)
            s_cur=np.copy(s_past)
            simul_obs = np.loadtxt('ContinueObs.txt')
            simul_obs = simul_obs.reshape(-1, 1)
        else:
            s_cur = np.copy(s_init)		
            s_past = np.copy(s_init)
            simul_obs = np.copy(simul_obs_init)
        #s_cur = np.copy(s_init) # marked by kxy
        #s_past = np.copy(s_init)
        ...
        for i in range(maxiter):
            ...
            dill.dump_session('./result' + str(i+1) + '.pkl') #kxy added
