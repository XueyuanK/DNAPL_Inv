...
prior_std = 1.5**0.5
prior_cov_scale = np.array([8., 8., 4.])

def kernel(r): return (prior_std ** 2) * np.exp(-r)  #true kernel
...
s_true = np.loadtxt('true_sntransform.txt')
#s_true = np.array(s_true).reshape(-1, 1)  # make it 2D array

obs = np.loadtxt('obs_error.txt')

PTT_params = params = {'nx': nx, 'ny': ny, 'nz': nz,}

def forward_model(s, parallelization, ncores=None):
    model = PTT_comsol.Model(PTT_params)
...
s_init = np.ones((m, 1))*(-3.66) # ini_sn=0.02
